...
from brewing_incubator.logging import get_logger, setup_logging

if TYPE_CHECKING:
    from collections.abc import Callable

    from brewing_incubator.db.database import Migrations, MigrationsProtocol
    from brewing_incubator.db.settings import PostgresqlSettings
    from brewing_incubator.http import BrewingHTTP


logger = get_logger()


def asgi_application_sting(factory: Callable[[], BrewingHTTP], /):
...
class AppCLI(CLI):
    def __init__(
        ...
        /,
        *children: CLI,
        api_factory: Callable[[], BrewingHTTP],
        typer: typer.Typer | None = None,
    ):
...
def build_cli(
    api_factory: Callable[[], BrewingHTTP],
    dev_api_factory: Callable[[], BrewingHTTP],
